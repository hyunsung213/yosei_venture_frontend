...
import { INews, News, PaginationInfo } from '@/interface/interface';
...
export default function PressPage() {
  ...
  const load = async (currentPage: number) => {
    setLoading(true);
    try {
      const data = await getAllNews(currentPage, 10);
      if (data) {
  ...
  const handlePageChange = (newPage: number) => {
    router.push(`/community/news?page=${newPage}`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      ...
      if (page === 1) {
        ...
        router.push(`/community/news?page=1`);
      }
    } else {
