...
import { ArrowLeft, Loader2, Lock, Edit2, CheckCircle2, MessageSquare, Trash2 } from "lucide-react";
...
export default function CommunityQADetailPage() {
  ...
  const handleEditClick = () => {
    if (verifiedPassword === qa?.pwd || role === "super") {
      setIsEditing(true);
    } else {
  ...
  if (error || !qa) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-32 flex flex-col items-center justify-center gap-4 text-red-500">
        ...
        <Link href="/community/qa" className="text-sm text-gray-500 hover:text-yonsei-blue underline transition-colors">
          목록으로 돌아가기
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 mt-12 md:mt-16 bg-white shadow-sm rounded-2xl border border-gray-100">
      <div className="mb-8 flex items-center justify-between">
        <Link href="/community/qa" className="inline-flex items-center gap-2 text-gray-500 hover:text-yonsei-blue transition-colors font-medium">
      ...
      {/* 수정 시 비밀번호 확인 모달 (공개 글 등 비밀번호 확인이 안된 상태에서 수정 시도 시) */}
      {showEditAuthPrompt && (
