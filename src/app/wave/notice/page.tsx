...
import { PlusCircle, X, Pin } from 'lucide-react';
...
function WaveNoticeContent() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await getWaveNotices(page, 10);
        if (data) {
  ...
  const handlePageChange = (newPage: number) => {
    router.push(`/wave/notice?page=${newPage}`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append("isWave", "true"); // Wave 공지사항임을 명시
    ...
    if (res.success) {
      ...
      if (page === 1) {
        ...
        router.push(`/wave/notice?page=1`);
      }
    } else {
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 border-t border-gray-100 flex justify-end gap-3 bg-white">
              ...
              <button 
                ...
                disabled={isPending}
                className="px-5 py-2.5 rounded-lg bg-yonsei-blue text-white font-bold hover:bg-blue-800 transition shadow-md disabled:opacity-50"
              >
                {isPending ? "생성 중..." : "생성하기"}
              </button>
            </div>
                ...
                [...notices].sort((a, b) => (b.isPinned ? 1 : 0) - (a.isPinned ? 1 : 0)).map((nt, idx) => (
                  <tr key={nt.id || idx} className={`transition-colors ${nt.isPinned ? 'bg-blue-50/50 hover:bg-blue-50' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="px-6 py-4">
                      <Link href={`/wave/notice/${nt.id}`} className={`font-bold transition-colors block truncate w-full max-w-lg mx-auto text-center ${nt.isPinned ? 'text-yonsei-blue' : 'text-gray-900 hover:text-yonsei-blue'}`}>
                        {nt.title}
                      </Link>
                    </td>
